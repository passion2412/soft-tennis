...
<head>
    ...
    <style>
        ...
        .btn {{ 
            ...
            color: white !important; font-weight: bold; font-size: 11px; text-align: center; line-height: 1.1;
...
<body>
    <!-- 1. 題名の固定表示を削除（試合名から開始） -->
    <div id="match-display" class="match-name">今日の試合</div>

    <div class="score-box">
        <div id="names-display" style="font-size: 12px; opacity: 0.8;">自分 & ペア</div>
    ...
    <div class="footer">
        <details>
            ...
            <input type="text" id="in-match" placeholder="試合名を入力" oninput="updateSettings()">
            <input type="text" id="in-p1" placeholder="自分（名前）" oninput="updateSettings()">
            <input type="text" id="in-p2" placeholder="ペア（名前）" oninput="updateSettings()">
            <button onclick="resetAll()" style="width:100%; padding:10px; background:#f44336; color:white; border:none; border-radius:4px; margin:10px 0;">全リセット</button>
...
# key=ver をつけることでキャッシュを強制回避
components.html(html_code, height=850, scrolling=True, key=f"counter_{ver}")
